...
std::vector<int> Callable::insert_ndarray_param(const DataType &dt,
  ...
  auto *type = TypeFactory::get_instance().get_ndarray_struct_type(dtype, ndim, needs_grad);
  auto p = Parameter(type, /*is_array=*/true, 0, ndim + element_shape.size(), element_shape, BufferFormat::unknown,
                     needs_grad);
  p.name = name;
...
std::vector<int> Callable::insert_pointer_param(const DataType &dt, const std::string &name) {
...
void Callable::finalize_rets() {
  ...
  auto *type = TypeFactory::get_instance().get_struct_type(members)->as<StructType>();
  std::string layout = program->get_kernel_return_data_layout();
  std::tie(ret_type, ret_size) = program->get_struct_type_with_data_layout(type, layout);
}
...
void Callable::finalize_params() {
  ...
  for (int i = 0; i < parameter_list.size(); i++) {
    ...
    members.push_back({param.is_array && !param.get_dtype()->is<StructType>()
  ...
  std::tie(args_type, args_size) = program->get_struct_type_with_data_layout(type, layout);
}
}  // namespace quadrants::lang
