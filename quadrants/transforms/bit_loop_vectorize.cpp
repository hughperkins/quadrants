...
class BitLoopVectorize : public IRVisitor {
 public:
  ...
  void visit(GlobalLoadStmt *stmt) override {
    ...
    if (in_struct_for_loop && is_bit_vectorized) {
      if (ptr_type->get_pointee_type()->cast<QuantIntType>()) {
        ...
        auto ptr_physical_type = TypeFactory::get_instance().get_pointer_type(quant_array_physical_type, false);
        ...
        if (ptr->indices.size() == 2) {
          ...
          if (diff.linear_related() && diff.certain() && (diff.low == 1 || diff.low == -1)) {
            // construct ptr to x[i, j]
            auto indices = ptr->indices;
            indices[1] = loop_stmt->body->statements[1].get();
            auto base_ptr = std::make_unique<GlobalPtrStmt>(ptr->snode, indices);
            ...
            auto offset_constant = std::make_unique<ConstStmt>(TypedConstant(vectorization_width));
            ...
            auto offset_ptr = std::make_unique<GlobalPtrStmt>(ptr->snode, indices);
            ...
            auto base_shift_offset = std::make_unique<ConstStmt>(TypedConstant(load_data_type, 1));
            auto base_shift_opcode = diff.low == -1 ? BinaryOpType::bit_shl : BinaryOpType::bit_sar;
            auto base_shift_op =
                std::make_unique<BinaryOpStmt>(base_shift_opcode, load_base.get(), base_shift_offset.get());

            auto offsetted_shift_offset =
                std::make_unique<ConstStmt>(TypedConstant(load_data_type, vectorization_width - 1));
            auto offsetted_shift_opcode = diff.low == -1 ? BinaryOpType::bit_sar : BinaryOpType::bit_shl;
            auto offsetted_shift_op = std::make_unique<BinaryOpStmt>(offsetted_shift_opcode, load_offsetted.get(),
                                                                     offsetted_shift_offset.get());

            auto or_op =
                std::make_unique<BinaryOpStmt>(BinaryOpType::bit_or, base_shift_op.get(), offsetted_shift_op.get());
            // modify IR
  ...
  void visit(BinaryOpStmt *stmt) override {
    ...
    if (in_struct_for_loop && is_bit_vectorized) {
      if (stmt->op_type == BinaryOpType::bit_and) {
        ...
        if (lhs_val == 1) {
          if (auto rhs = stmt->rhs->cast<BinaryOpStmt>(); rhs && rhs->is_bit_vectorized) {
        ...
        if (auto lhs = stmt->lhs->cast<GlobalLoadStmt>()) {
          ...
          if (auto ptr = lhs->src->cast<GlobalPtrStmt>(); ptr && ptr->is_bit_vectorized) {
            ...
            auto add = std::make_unique<BinaryOpStmt>(BinaryOpType::add, stmt->lhs, zero.get());
          ...
          if (it != transformed_atomics.end()) {
            ...
            // compute not_a first
            auto not_a = std::make_unique<UnaryOpStmt>(UnaryOpType::bit_not, load_a.get());
            // b should always be itself so do nothing
            // compute not_c
            auto not_c = std::make_unique<UnaryOpStmt>(UnaryOpType::bit_not, load_c.get());
            // bit_and all three patterns
            auto and_a_b = std::make_unique<BinaryOpStmt>(BinaryOpType::bit_and, not_a.get(), load_b.get());
            auto and_b_c = std::make_unique<BinaryOpStmt>(
                BinaryOpType::bit_and, and_a_b.get(), rhs_val == 2 ? (Stmt *)(not_c.get()) : (Stmt *)(load_c.get()));
  ...
  void visit(AtomicOpStmt *stmt) override {
    ...
    if (in_struct_for_loop && is_bit_vectorized && stmt->op_type == AtomicOpType::add) {
 ...
 private:
  ...
  int32 get_constant_value(Stmt *stmt) {
    ...
    if (auto cast_stmt = stmt->cast<UnaryOpStmt>();
        cast_stmt && cast_stmt->is_cast() && cast_stmt->op_type == UnaryOpType::cast_value) {
      stmt = cast_stmt->operand;
    }
    if (auto constant_stmt = stmt->cast<ConstStmt>();
        constant_stmt && constant_stmt->val.dt->is_primitive(PrimitiveTypeID::i32)) {
