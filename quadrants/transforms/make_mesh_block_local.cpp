...
void MakeMeshBlockLocal::simplify_nested_conversion() {
  ...
  irpass::analysis::gather_statements(offload_->body.get(), [&](Stmt *stmt) {
    if (auto conv1 = stmt->cast<MeshIndexConversionStmt>()) {
      if (auto conv2 = conv1->idx->cast<MeshIndexConversionStmt>()) {
        if (conv1->conv_type == mesh::ConvType::g2r && conv2->conv_type == mesh::ConvType::l2g &&
            conv1->mesh == conv2->mesh && conv1->idx_type == conv2->idx_type) {  // nested
  ...
  for (size_t i = 0; i < stmts.size(); ++i) {
    stmts[i]->replace_with(
        Stmt::make<MeshIndexConversionStmt>(stmts[i]->mesh, stmts[i]->idx_type, ori_indices[i], mesh::ConvType::l2r));
  }
}
    ...
    if (auto conv = stmt->cast<MeshIndexConversionStmt>()) {
      if (conv->conv_type != mesh::ConvType::g2r) {
        ...
        for (auto rel : offload_->minor_relation_types) {
          auto from_type = mesh::MeshElementType(mesh::from_end_element_order(rel));
...
void MakeMeshBlockLocal::replace_conv_statements() {
  ...
  for (auto stmt : idx_conv_stmts) {
    VecStatement bls;
    Stmt *bls_element_offset_bytes = bls.push_back<ConstStmt>(TypedConstant{(int32)mapping_bls_offset_in_bytes_});
    Stmt *idx_byte = bls.push_back<BinaryOpStmt>(BinaryOpType::mul, stmt->idx,
                                                 bls.push_back<ConstStmt>(TypedConstant(mapping_dtype_size_)));
    Stmt *offset = bls.push_back<BinaryOpStmt>(BinaryOpType::add, bls_element_offset_bytes, idx_byte);
    Stmt *bls_ptr =
        bls.push_back<BlockLocalPtrStmt>(offset, TypeFactory::get_instance().get_pointer_type(mapping_data_type_));
...
void MakeMeshBlockLocal::replace_global_ptrs(SNode *snode) {
  ...
  irpass::analysis::gather_statements(offload_->body.get(), [&](Stmt *stmt) {
    if (auto global_ptr = stmt->cast<GlobalPtrStmt>()) {
      if (global_ptr->snode == snode && global_ptr->indices[0]->is<MeshIndexConversionStmt>()) {
  ...
  for (auto global_ptr : global_ptrs) {
    VecStatement bls;
    Stmt *local_idx = global_ptr->indices[0]->as<MeshIndexConversionStmt>()->idx;
    Stmt *local_idx_byte =
        bls.push_back<BinaryOpStmt>(BinaryOpType::mul, local_idx, bls.push_back<ConstStmt>(TypedConstant(dtype_size)));
    Stmt *offset = bls.push_back<ConstStmt>(TypedConstant(int32(offset_in_bytes)));
    Stmt *index = bls.push_back<BinaryOpStmt>(BinaryOpType::add, offset, local_idx_byte);
    [[maybe_unused]] Stmt *bls_ptr =
        bls.push_back<BlockLocalPtrStmt>(index, TypeFactory::get_instance().get_pointer_type(data_type));
  ...
  irpass::analysis::gather_statements(offload_->body.get(), [&](Stmt *stmt) {
    if (auto atomic_op = stmt->cast<AtomicOpStmt>()) {
      if (atomic_op->op_type == AtomicOpType::add && atomic_op->dest->is<BlockLocalPtrStmt>()) {
  ...
  for (auto atomic_op : atomic_ops) {
    ...
    Stmt *res_val = non_atomic.push_back<BinaryOpStmt>(BinaryOpType::add, dest_val, atomic_op->val);
...
//  i += blockDim.x;
// }
Stmt *MakeMeshBlockLocal::create_xlogue(Stmt *start_val,
                                        Stmt *end_val,
                                        std::function<void(Block * /*block*/, Stmt * /*idx_val*/)> body_) {
  Stmt *idx = block_->push_back<AllocaStmt>(mapping_data_type_);
  [[maybe_unused]] Stmt *init_val = block_->push_back<LocalStoreStmt>(idx, start_val);
  Stmt *block_dim_val;
  if (config_.arch == Arch::x64 || config_.arch == Arch::arm64) {
    block_dim_val = block_->push_back<ConstStmt>(TypedConstant(1));
  } else {
    block_dim_val = block_->push_back<ConstStmt>(TypedConstant{offload_->block_dim});
  }
...
Stmt *MakeMeshBlockLocal::create_cache_mapping(
  ...
  Stmt *bls_element_offset_bytes = block_->push_back<ConstStmt>(TypedConstant{(int32)mapping_bls_offset_in_bytes_});
  return create_xlogue(start_val, end_val, [&](Block *body, Stmt *idx_val) {
    ...
    Stmt *offset = body->push_back<BinaryOpStmt>(BinaryOpType::add, bls_element_offset_bytes, idx_val_byte);
    Stmt *bls_ptr =
        body->push_back<BlockLocalPtrStmt>(offset, TypeFactory::get_instance().get_pointer_type(mapping_data_type_));
    Stmt *casted_val = body->push_back<UnaryOpStmt>(UnaryOpType::cast_value, global_val(body, idx_val));
    casted_val->as<UnaryOpStmt>()->cast_type = PrimitiveType::i32;
    [[maybe_unused]] Stmt *bls_store = body->push_back<GlobalStoreStmt>(bls_ptr, casted_val);
  });
}

void MakeMeshBlockLocal::fetch_attr_to_bls(Block *body, Stmt *idx_val, Stmt *mapping_val) {
  ...
  for (auto [snode, total_flags] : attrs->second) {
    ...
    QD_ASSERT_INFO(!(bls_has_accumulate && bls_has_read), "BLS with both read and accumulation is not supported.");

    bool first_allocate = {false};
    if (attr_bls_offset_in_bytes_.find(snode) == attr_bls_offset_in_bytes_.end()) {
      first_allocate = {true};
      bls_offset_in_bytes_ += (dtype_size - bls_offset_in_bytes_ % dtype_size) % dtype_size;
      attr_bls_offset_in_bytes_.insert(std::make_pair(snode, bls_offset_in_bytes_));
      bls_offset_in_bytes_ += dtype_size * offload_->mesh->patch_max_element_num.find(element_type_)->second;
    }
    auto offset_in_bytes = attr_bls_offset_in_bytes_.find(snode)->second;
    ...
    if (bls_has_read) {
      ...
      Stmt *global_ptr = body->push_back<GlobalPtrStmt>(snode, std::vector<Stmt *>{mapping_val});
    ...
    Stmt *offset = body->push_back<ConstStmt>(TypedConstant(int32(offset_in_bytes)));
    Stmt *idx_val_byte = body->push_back<BinaryOpStmt>(BinaryOpType::mul, idx_val,
    ...
    Stmt *bls_ptr = body->push_back<BlockLocalPtrStmt>(index, TypeFactory::get_instance().get_pointer_type(data_type));
...
void MakeMeshBlockLocal::push_attr_to_global(Block *body, Stmt *idx_val, Stmt *mapping_val) {
  auto attrs = rec_.find(std::make_pair(element_type_, conv_type_));
  if (attrs == rec_.end()) {
    return;
  }
...
void MakeMeshBlockLocal::fetch_mapping(
    std::function<Stmt *(Stmt * /*start_val*/,
                         Stmt * /*end_val*/,
                         std::function<Stmt *(Block * /*block*/, Stmt * /*idx_val*/)>)> mapping_callback_handler,
  ...
  if (config_.arch == Arch::x64 || config_.arch == Arch::arm64) {
    ...
    thread_idx_stmt = block_->push_back<LoopLinearIndexStmt>(offload_);  // Equivalent to CUDA threadIdx
  }
  ...
  if (config_.optimize_mesh_reordered_mapping && conv_type_ == mesh::ConvType::l2r) {
    // int i = threadIdx.x;
    // while (i < owned_{}_num) {
    ...
    //  i += blockDim.x;
    // }
    Stmt *owned_element_num = offload_->owned_num_local.find(element_type_)->second;
    Stmt *owned_element_offset = offload_->owned_offset_local.find(element_type_)->second;
    Stmt *pre_idx_val = mapping_callback_handler(thread_idx_stmt, owned_element_num, [&](Block *body, Stmt *idx_val) {
      Stmt *global_index = body->push_back<BinaryOpStmt>(BinaryOpType::add, idx_val, owned_element_offset);
      attr_callback_handler(body, idx_val, global_index);
      return global_index;
    });
    mapping_callback_handler(pre_idx_val, total_element_num, [&](Block *body, Stmt *idx_val) {
      Stmt *global_offset = body->push_back<BinaryOpStmt>(BinaryOpType::add, total_element_offset, idx_val);
      Stmt *global_ptr = body->push_back<GlobalPtrStmt>(mapping_snode_, std::vector<Stmt *>{global_offset});
      Stmt *global_load = body->push_back<GlobalLoadStmt>(global_ptr);
      Stmt *casted_global_load = body->push_back<UnaryOpStmt>(UnaryOpType::cast_value, global_load);
    ...
    //  i += blockDim.x;
    // }
    mapping_callback_handler(thread_idx_stmt, total_element_num, [&](Block *body, Stmt *idx_val) {
      Stmt *global_offset = body->push_back<BinaryOpStmt>(BinaryOpType::add, total_element_offset, idx_val);
      Stmt *global_ptr = body->push_back<GlobalPtrStmt>(mapping_snode_, std::vector<Stmt *>{global_offset});
      Stmt *global_load = body->push_back<GlobalLoadStmt>(global_ptr);
      Stmt *casted_global_load = body->push_back<UnaryOpStmt>(UnaryOpType::cast_value, global_load);
...
MakeMeshBlockLocal::MakeMeshBlockLocal(OffloadedStmt *offload, const CompileConfig &config)
  ...
  if (offload->major_to_types.size() != 1 ||       // not support multiple major relations yet
      ...
      offload->mem_access_opt.get_snodes_with_flag(SNodeAccessFlag::mesh_local).size() >
          0) {  // disable when user determine which attributes to
                // be cached manually
  ...
  if (auto_mesh_local && config.arch == Arch::cuda) {
    ...
    std::sort(priority_caches.begin(), priority_caches.end(), [](const MeshBLSCache &a, const MeshBLSCache &b) {
      return a.total_flags > b.total_flags || (a.total_flags == b.total_flags && a.loop_index > b.loop_index) ||
    ...
    for (const auto &cache : priority_caches) {
      ...
      QD_TRACE("available = {}, x = {}, loop_index = {}, unique_access = {}", available_bytes, int(cache.total_flags),
               int(cache.loop_index), cache.unique_accessed);
  ...
  auto has_acc = [&](mesh::MeshElementType element_type, mesh::ConvType conv_type) {
  ...
  for (auto [element_type, conv_type] : mappings_) {
    ...
    if (offload->total_offset_local.find(element_type) == offload->total_offset_local.end()) {
      continue;
    }

    mapping_snode_ = (offload->mesh->index_mapping.find(std::make_pair(element_type, conv_type))->second);
    ...
    bls_offset_in_bytes_ += (mapping_dtype_size_ - bls_offset_in_bytes_ % mapping_dtype_size_) % mapping_dtype_size_;
    mapping_bls_offset_in_bytes_ = bls_offset_in_bytes_;
    // allocate storage for the BLS variable
    bls_offset_in_bytes_ += mapping_dtype_size_ * offload->mesh->patch_max_element_num.find(element_type)->second;
    ...
    fetch_mapping(
        [&](Stmt *start_val, Stmt *end_val, std::function<Stmt *(Block * /*block*/, Stmt * /*idx_val*/)> global_val) {
          return create_cache_mapping(start_val, end_val, global_val);
        },
        [&](Block *body, Stmt *idx_val, Stmt *mapping_val) { fetch_attr_to_bls(body, idx_val, mapping_val); });
    ...
    {
      Stmt *thread_idx_stmt = block_->push_back<LoopLinearIndexStmt>(offload);  // Equivalent to CUDA threadIdx
      Stmt *total_element_num = offload->total_num_local.find(element_type)->second;
      [[maybe_unused]] Stmt *total_element_offset = offload->total_offset_local.find(element_type)->second;
      create_xlogue(thread_idx_stmt, total_element_num, [&](Block *body, Stmt *idx_val) {
        Stmt *bls_element_offset_bytes = body->push_back<ConstStmt>(TypedConstant{(int32)mapping_bls_offset_in_bytes_});
        Stmt *idx_byte = body->push_back<BinaryOpStmt>(BinaryOpType::mul, idx_val,
                                                       body->push_back<ConstStmt>(TypedConstant(mapping_dtype_size_)));
        Stmt *offset = body->push_back<BinaryOpStmt>(BinaryOpType::add, bls_element_offset_bytes, idx_byte);
  ...
  for (auto [mapping, attr_set] : rec_) {
    ...
    mapping_snode_ = (offload->mesh->index_mapping.find(std::make_pair(element_type_, conv_type_))->second);
    mapping_data_type_ = mapping_snode_->dt.ptr_removed();
    mapping_dtype_size_ = data_type_size(mapping_data_type_);
    ...
    fetch_mapping(
        [&](Stmt *start_val, Stmt *end_val, std::function<Stmt *(Block * /*block*/, Stmt * /*idx_val*/)> global_val) {
          return create_xlogue(start_val, end_val, [&](Block *block, Stmt *idx_val) { global_val(block, idx_val); });
        },
        [&](Block *body, Stmt *idx_val, Stmt *mapping_val) { fetch_attr_to_bls(body, idx_val, mapping_val); });
    ...
    fetch_mapping(
        ...
        [&](Block *body, Stmt *idx_val, Stmt *mapping_val) { push_attr_to_global(body, idx_val, mapping_val); });
  }

  offload->bls_size = std::max(std::size_t(1), bls_offset_in_bytes_);
}

void MakeMeshBlockLocal::run(OffloadedStmt *offload, const CompileConfig &config, const std::string &kernel_name) {
...
void make_mesh_block_local(IRNode *root, const CompileConfig &config, const MakeMeshBlockLocal::Args &args) {
  ...
  if (auto root_block = root->cast<Block>()) {
    for (auto &offload : root_block->statements) {
      MakeMeshBlockLocal::run(offload->cast<OffloadedStmt>(), config, args.kernel_name);
    }
  } else {
    MakeMeshBlockLocal::run(root->as<OffloadedStmt>(), config, args.kernel_name);
  }
