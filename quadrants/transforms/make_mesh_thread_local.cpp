...
void make_mesh_thread_local_offload(OffloadedStmt *offload,
  ...
  auto patch_idx = offload->tls_prologue->insert(std::make_unique<MeshPatchIndexStmt>(), -1);
  auto one = offload->tls_prologue->insert(std::make_unique<ConstStmt>(TypedConstant(PrimitiveType::i32, 1)), -1);
  auto patch_idx_1 =
      offload->tls_prologue->insert(std::make_unique<BinaryOpStmt>(BinaryOpType::add, patch_idx, one), -1);
  ...
  auto make_thread_local_store = [&](mesh::MeshElementType element_type,
    ...
    // Create thread local storage
    {
      ...
      auto offset_globalptr = offload->tls_prologue->insert(
          std::make_unique<GlobalPtrStmt>(offset_snode->second, std::vector<Stmt *>{patch_idx}), -1);
      auto offset_load = offload->tls_prologue->insert(std::make_unique<GlobalLoadStmt>(offset_globalptr), -1);
      auto offset_1_globalptr = offload->tls_prologue->insert(
          std::make_unique<GlobalPtrStmt>(offset_snode->second, std::vector<Stmt *>{patch_idx_1}), -1);
      auto offset_1_load = offload->tls_prologue->insert(std::make_unique<GlobalLoadStmt>(offset_1_globalptr), -1);
    ...
    {
      auto offset_ptr = offload->mesh_prologue->push_back<ThreadLocalPtrStmt>(
          offset_tls_offset, TypeFactory::get_instance().get_pointer_type(data_type));
      auto _offset_val = offload->mesh_prologue->push_back<GlobalLoadStmt>(offset_ptr);
      ...
      auto num_ptr = offload->mesh_prologue->push_back<ThreadLocalPtrStmt>(
          num_tls_offset, TypeFactory::get_instance().get_pointer_type(data_type));
      auto _num_val = offload->mesh_prologue->push_back<GlobalLoadStmt>(num_ptr);
  ...
  for (auto element_type : accessed.first) {
    make_thread_local_store(element_type, offload->mesh->owned_offset, offload->owned_offset_local,
                            offload->owned_num_local);
  }

  for (auto element_type : accessed.second) {
    make_thread_local_store(element_type, offload->mesh->total_offset, offload->total_offset_local,
...
void make_mesh_thread_local(IRNode *root, const CompileConfig &config, const MakeBlockLocalPass::Args &args) {
  ...
  if (auto root_block = root->cast<Block>()) {
    for (auto &offload : root_block->statements) {
      make_mesh_thread_local_offload(offload->cast<OffloadedStmt>(), config, args.kernel_name);
    }
  } else {
    make_mesh_thread_local_offload(root->as<OffloadedStmt>(), config, args.kernel_name);
  }
