...
bool check_cupti_availability() {
  ...
  CUDADriver::get_instance().device_get_attribute(&cc_major, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR, device);
  if (cc_major < 7) {
    QD_WARN(
...
#define TEMP_CUPQD_API_CALL(api_func_call)                                                                     \
  do {                                                                                                         \
    ...
    if (status != CUPQD_SUCCESS) {                                                                             \
      ...
      fprintf(stderr, "%s:%d: error: function %s failed with error %s.\n", __FILE__, __LINE__, #api_func_call, \
              errstr);                                                                                         \
      exit(-1);                                                                                                \
    }                                                                                                          \
  } while (0)

  CUpti_Profiler_Initialize_Params profiler_initialize_params = {CUpti_Profiler_Initialize_Params_STRUCT_SIZE};
  ...
  CUpti_Device_GetChipName_Params get_chip_name_params = {CUpti_Device_GetChipName_Params_STRUCT_SIZE};
  ...
  get_counter_availability_params.ctx = (CUcontext)(CUDAContext::get_instance().get_context());
  TEMP_CUPQD_API_CALL(cuptiProfilerGetCounterAvailability(&get_counter_availability_params));

#undef TEMP_CUPQD_API_CALL

  std::vector<uint8_t> counter_availability_image;
  counter_availability_image.clear();
  counter_availability_image.resize(get_counter_availability_params.counterAvailabilityImageSize);
  get_counter_availability_params.pCounterAvailabilityImage = counter_availability_image.data();
  CUptiResult status = cuptiProfilerGetCounterAvailability(&get_counter_availability_params);
...
#define SCOPE_EXIT(func) \
  const auto NV_ANONYMOUS_VARIABLE_INDIRECT(EXIT, __LINE__) = MoveScopeExit([=]() { (void)(func); })
...
#define RETURN_IF_NVPW_ERROR(retval, actual)                                                  \
  do {                                                                                        \
    ...
    if (NVPA_STATUS_SUCCESS != status) {                                                      \
      fprintf(stderr, "FAILED: %s with error %s\n", #actual, get_nvpw_result_string(status)); \
      return retval;                                                                          \
    }                                                                                         \
  } while (0)
...
bool get_raw_metric_requests(std::string chip_name,
  ...
  calculate_scratch_buffer_size_param.pCounterAvailabilityImage = p_counter_availability_image;
  RETURN_IF_NVPW_ERROR(false,
                       NVPW_CUDA_MetricsEvaluator_CalculateScratchBufferSize(&calculate_scratch_buffer_size_param));

  std::vector<uint8_t> scratch_buffer(calculate_scratch_buffer_size_param.scratchBufferSize);
  ...
  metric_evaluator_initialize_params.pCounterAvailabilityImage = p_counter_availability_image;
  RETURN_IF_NVPW_ERROR(false, NVPW_CUDA_MetricsEvaluator_Initialize(&metric_evaluator_initialize_params));
  NVPW_MetricsEvaluator *metric_evaluator = metric_evaluator_initialize_params.pMetricsEvaluator;
  ...
  for (auto &metric_name : metric_names) {
    std::string req_name;
    parse_metric_name_string(metric_name, &req_name, &isolated, &keep_instances);
    keep_instances = true;
    ...
    NVPW_MetricsEvaluator_GetMetricRawDependencies_Params get_metric_raw_dependencies_parms = {
    ...
    get_metric_raw_dependencies_parms.pMetricEvalRequests = &metric_eval_request;
    get_metric_raw_dependencies_parms.numMetricEvalRequests = 1;
    get_metric_raw_dependencies_parms.metricEvalRequestStructSize = NVPW_MetricEvalRequest_STRUCT_SIZE;
    get_metric_raw_dependencies_parms.metricEvalRequestStrideSize = sizeof(NVPW_MetricEvalRequest);
    RETURN_IF_NVPW_ERROR(false, NVPW_MetricsEvaluator_GetMetricRawDependencies(&get_metric_raw_dependencies_parms));
    raw_dependencies.resize(get_metric_raw_dependencies_parms.numRawDependencies);
    get_metric_raw_dependencies_parms.ppRawDependencies = raw_dependencies.data();
    RETURN_IF_NVPW_ERROR(false, NVPW_MetricsEvaluator_GetMetricRawDependencies(&get_metric_raw_dependencies_parms));
  ...
  for (auto &raw_metric_name : raw_metric_names) {
    NVPA_RawMetricRequest metric_request = {NVPA_RAW_METRIC_REQUEST_STRUCT_SIZE};
  ...
  RETURN_IF_NVPW_ERROR(false, NVPW_MetricsEvaluator_Destroy(&metric_evaluator_destroy_params));
  return true;
}
...
bool get_config_image(std::string chip_name,
                      ...
                      const uint8_t *p_counter_availability_image) {
  std::vector<NVPA_RawMetricRequest> raw_metric_requests;
  get_raw_metric_requests(chip_name, metric_names, raw_metric_requests, p_counter_availability_image);
  ...
  raw_metrics_config_create_params.pCounterAvailabilityImage = p_counter_availability_image;
  RETURN_IF_NVPW_ERROR(false, NVPW_CUDA_RawMetricsConfig_Create_V2(&raw_metrics_config_create_params));
  NVPA_RawMetricsConfig *p_raw_metrics_config = raw_metrics_config_create_params.pRawMetricsConfig;

  if (p_counter_availability_image) {
    NVPW_RawMetricsConfig_SetCounterAvailability_Params set_counter_availability_params = {
    ...
    set_counter_availability_params.pCounterAvailabilityImage = p_counter_availability_image;
    RETURN_IF_NVPW_ERROR(false, NVPW_RawMetricsConfig_SetCounterAvailability(&set_counter_availability_params));
  }
  ...
  SCOPE_EXIT([&]() {
    NVPW_RawMetricsConfig_Destroy((NVPW_RawMetricsConfig_Destroy_Params *)&raw_metrics_config_destroy_params);
  });
  ...
  RETURN_IF_NVPW_ERROR(false, NVPW_RawMetricsConfig_BeginPassGroup(&begin_pass_group_params));

  NVPW_RawMetricsConfig_AddMetrics_Params add_metrics_params = {NVPW_RawMetricsConfig_AddMetrics_Params_STRUCT_SIZE};
  ...
  RETURN_IF_NVPW_ERROR(false, NVPW_RawMetricsConfig_AddMetrics(&add_metrics_params));
  ...
  RETURN_IF_NVPW_ERROR(false, NVPW_RawMetricsConfig_EndPassGroup(&end_pass_group_params));

  NVPW_RawMetricsConfig_GenerateConfigImage_Params generate_config_image_params = {
  ...
  RETURN_IF_NVPW_ERROR(false, NVPW_RawMetricsConfig_GenerateConfigImage(&generate_config_image_params));
  ...
  RETURN_IF_NVPW_ERROR(false, NVPW_RawMetricsConfig_GetConfigImage(&get_config_image_params));
...
// copy from : CUPTI/samples/extensions/src/profilerhost_util/Metric.cpp
bool get_counter_data_prefix_image(std::string chip_name,
                                   const std::vector<std::string> &metric_names,
  ...
  NVPW_CUDA_CounterDataBuilder_Create_Params counter_data_builder_create_params = {
      NVPW_CUDA_CounterDataBuilder_Create_Params_STRUCT_SIZE};
  counter_data_builder_create_params.pChipName = chip_name.c_str();
  counter_data_builder_create_params.pCounterAvailabilityImage = p_counter_availability_image;
  RETURN_IF_NVPW_ERROR(false, NVPW_CUDA_CounterDataBuilder_Create(&counter_data_builder_create_params));
  ...
  counter_data_builder_destroy_params.pCounterDataBuilder = counter_data_builder_create_params.pCounterDataBuilder;
  SCOPE_EXIT([&]() {
    NVPW_CounterDataBuilder_Destroy((NVPW_CounterDataBuilder_Destroy_Params *)&counter_data_builder_destroy_params);
  });

  NVPW_CounterDataBuilder_AddMetrics_Params add_metrics_params = {
      NVPW_CounterDataBuilder_AddMetrics_Params_STRUCT_SIZE};
  add_metrics_params.pCounterDataBuilder = counter_data_builder_create_params.pCounterDataBuilder;
  add_metrics_params.pRawMetricRequests = raw_metric_requests.data();
  add_metrics_params.numMetricRequests = raw_metric_requests.size();
  RETURN_IF_NVPW_ERROR(false, NVPW_CounterDataBuilder_AddMetrics(&add_metrics_params));

  NVPW_CounterDataBuilder_GetCounterDataPrefix_Params get_counter_dataPrefix_params = {
      NVPW_CounterDataBuilder_GetCounterDataPrefix_Params_STRUCT_SIZE};
  get_counter_dataPrefix_params.pCounterDataBuilder = counter_data_builder_create_params.pCounterDataBuilder;
  get_counter_dataPrefix_params.bytesAllocated = 0;  // size_t counterDataPrefixSize = 0
  get_counter_dataPrefix_params.pBuffer = NULL;
  RETURN_IF_NVPW_ERROR(false, NVPW_CounterDataBuilder_GetCounterDataPrefix(&get_counter_dataPrefix_params));

  counter_data_image_prefix.resize(get_counter_dataPrefix_params.bytesCopied);
  get_counter_dataPrefix_params.bytesAllocated = counter_data_image_prefix.size();
...
// copy from : CUPTI/samples/extensions/src/profilerhost_util/Metric.cpp
bool get_raw_metric_requests(NVPA_MetricsContext *metrics_context,
  ...
  for (auto &metric_name : metric_names) {
     ...
     * to true*/
    keep_instances = true;
    NVPW_MetricsContext_GetMetricProperties_Begin_Params get_metric_properties_begin_params = {
    ...
    RETURN_IF_NVPW_ERROR(false, NVPW_MetricsContext_GetMetricProperties_Begin(&get_metric_properties_begin_params));

    for (const char **pp_metric_dependencies = get_metric_properties_begin_params.ppRawMetricDependencies;
         *pp_metric_dependencies; ++pp_metric_dependencies) {
      temp.push_back(*pp_metric_dependencies);
    }
    NVPW_MetricsContext_GetMetricProperties_End_Params get_metric_properties_end_params = {
    ...
    RETURN_IF_NVPW_ERROR(false, NVPW_MetricsContext_GetMetricProperties_End(&get_metric_properties_end_params));
  }
...
bool get_config_image(std::string chip_name,
  ...
  RETURN_IF_NVPW_ERROR(false, NVPW_CUDA_MetricsContext_Create(&metrics_context_create_params));

  NVPW_MetricsContext_Destroy_Params metrics_context_destroy_params = {NVPW_MetricsContext_Destroy_Params_STRUCT_SIZE};
  metrics_context_destroy_params.pMetricsContext = metrics_context_create_params.pMetricsContext;
  SCOPE_EXIT(
      [&]() { NVPW_MetricsContext_Destroy((NVPW_MetricsContext_Destroy_Params *)&metrics_context_destroy_params); });

  std::vector<NVPA_RawMetricRequest> raw_metric_requests;
  std::vector<std::string> temp;
  get_raw_metric_requests(metrics_context_create_params.pMetricsContext, metric_names, raw_metric_requests, temp);

  NVPA_RawMetricsConfigOptions metrics_config_options = {NVPA_RAW_METRICS_CONFIG_OPTIONS_STRUCT_SIZE};
  ...
  RETURN_IF_NVPW_ERROR(false, NVPA_RawMetricsConfig_Create(&metrics_config_options, &p_raw_metrics_config));
...
bool get_counter_data_prefix_image(std::string chip_name,
  ...
  counter_data_builder_create_params.pChipName = chip_name.c_str();
  RETURN_IF_NVPW_ERROR(false, NVPW_CounterDataBuilder_Create(&counter_data_builder_create_params));
  ...
  NVPW_CounterDataBuilder_GetCounterDataPrefix_Params get_counter_data_prefix_params = {
      NVPW_CounterDataBuilder_GetCounterDataPrefix_Params_STRUCT_SIZE};
  get_counter_data_prefix_params.pCounterDataBuilder = counter_data_builder_create_params.pCounterDataBuilder;
  get_counter_data_prefix_params.bytesAllocated = 0;
  get_counter_data_prefix_params.pBuffer = NULL;
  RETURN_IF_NVPW_ERROR(false, NVPW_CounterDataBuilder_GetCounterDataPrefix(&get_counter_data_prefix_params));

  counter_data_image_prefix.resize(get_counter_data_prefix_params.bytesCopied);

  get_counter_data_prefix_params.bytesAllocated = counter_data_image_prefix.size();
...
bool create_counter_data_image(uint32_t num_ranges,
  ...
  counter_data_image_options.counterDataPrefixSize = counter_data_image_prefix.size();
  ...
  CUPQD_API_CALL(cuptiProfilerCounterDataImageCalculateSize(&calculate_size_params));
  ...
  initialize_params.counterDataImageSize = calculate_size_params.counterDataImageSize;
  ...
  scratch_buffer_size_params.counterDataImageSize = calculate_size_params.counterDataImageSize;
  scratch_buffer_size_params.pCounterDataImage = initialize_params.pCounterDataImage;
  CUPQD_API_CALL(cuptiProfilerCounterDataImageCalculateScratchBufferSize(&scratch_buffer_size_params));

  counter_data_scratch_buffer.resize(scratch_buffer_size_params.counterDataScratchBufferSize);
  ...
  init_scratch_buffer_params.counterDataImageSize = calculate_size_params.counterDataImageSize;

  init_scratch_buffer_params.pCounterDataImage = initialize_params.pCounterDataImage;
  init_scratch_buffer_params.counterDataScratchBufferSize = scratch_buffer_size_params.counterDataScratchBufferSize;
  init_scratch_buffer_params.pCounterDataScratchBuffer = &counter_data_scratch_buffer[0];

  CUPQD_API_CALL(cuptiProfilerCounterDataImageInitializeScratchBuffer(&init_scratch_buffer_params));
...
CuptiToolkit::CuptiToolkit() {
  ...
  uint metric_list_size = static_cast<uint>(CuptiMetricsDefault::CUPQD_METRIC_DEFAULT_TOTAL);
...
bool CuptiToolkit::init_cupti() {
  ...
  CUpti_Profiler_GetCounterAvailability_Params get_counter_availability_params = {
      CUpti_Profiler_GetCounterAvailability_Params_STRUCT_SIZE};
  get_counter_availability_params.ctx = (CUcontext)(CUDAContext::get_instance().get_context());
  CUPQD_API_CALL(cuptiProfilerGetCounterAvailability(&get_counter_availability_params));

  QD_TRACE("counterAvailabilityImageSize : {}",
           get_counter_availability_params.counterAvailabilityImageSize);  // 2192
  cupti_image_.counter_availability_image.clear();
  cupti_image_.counter_availability_image.resize(get_counter_availability_params.counterAvailabilityImageSize);
  get_counter_availability_params.pCounterAvailabilityImage = cupti_image_.counter_availability_image.data();
  CUPQD_API_CALL(cuptiProfilerGetCounterAvailability(&get_counter_availability_params));
...
#undef NVPW_API_CALL
  ...
  bool state = 0;
  state = get_config_image(cupti_image_.chip_name, cupti_config_.metric_list, cupti_image_.config_image,
  ...
  state = get_counter_data_prefix_image(cupti_image_.chip_name, cupti_config_.metric_list,
  ...
  state = create_counter_data_image(cupti_config_.num_ranges, cupti_image_.counter_data_image,
                                    cupti_image_.counter_data_scratch_buffer, cupti_image_.counter_data_image_prefix);
...
bool CuptiToolkit::begin_profiling() {
  ...
  begin_session_params.counterDataImageSize = cupti_image_.counter_data_image.size();
  begin_session_params.pCounterDataImage = &(cupti_image_.counter_data_image[0]);
  begin_session_params.counterDataScratchBufferSize = cupti_image_.counter_data_scratch_buffer.size();
  begin_session_params.pCounterDataScratchBuffer = &(cupti_image_.counter_data_scratch_buffer[0]);
...
bool CuptiToolkit::deinit_cupti() {
  CUpti_Profiler_DeInitialize_Params profiler_deinitialize_params = {CUpti_Profiler_DeInitialize_Params_STRUCT_SIZE};
  CUPQD_API_CALL(cuptiProfilerDeInitialize(&profiler_deinitialize_params));
  return true;
}

bool CuptiToolkit::update_record(uint32_t records_size_after_sync,
  ...
  NVPW_CounterData_GetNumRanges_Params get_num_ranges_params = {NVPW_CounterData_GetNumRanges_Params_STRUCT_SIZE};
  get_num_ranges_params.pCounterDataImage = &(cupti_image_.counter_data_image[0]);
  RETURN_IF_NVPW_ERROR(false, NVPW_CounterData_GetNumRanges(&get_num_ranges_params));
  ...
  for (size_t metric_index = 0; metric_index < cupti_config_.metric_list.size(); ++metric_index) {
    bool status = parse_metric_name_string(cupti_config_.metric_list[metric_index], &req_name[metric_index], &isolated,
                                           &keep_instances);
    if (!status) {
  ...
  QD_TRACE("get_num_ranges_params.numRanges = {}", get_num_ranges_params.numRanges);
  for (size_t range_index = 0; range_index < get_num_ranges_params.numRanges; ++range_index) {
    ...
    get_range_desc_params.pCounterDataImage = &(cupti_image_.counter_data_image[0]);
    get_range_desc_params.rangeIndex = range_index;
    RETURN_IF_NVPW_ERROR(false, NVPW_Profiler_CounterData_GetRangeDescriptions(&get_range_desc_params));
    ...
    for (size_t description_index = 0; description_index < get_range_desc_params.numDescriptions; ++description_index) {
    ...
    set_counter_data_params.pMetricsContext = metrics_context_create_params.pMetricsContext;
    set_counter_data_params.pCounterDataImage = &(cupti_image_.counter_data_image[0]);
    set_counter_data_params.isolated = true;
    set_counter_data_params.rangeIndex = range_index;
    RETURN_IF_NVPW_ERROR(false, NVPW_MetricsContext_SetCounterData(&set_counter_data_params));
    ...
    eval_to_gpu_params.pMetricsContext = metrics_context_create_params.pMetricsContext;
    ...
    RETURN_IF_NVPW_ERROR(false, NVPW_MetricsContext_EvaluateToGpuValues(&eval_to_gpu_params));

    // default metric : kernel_elapsed_time_in_ms
    double kernel_elapsed_clk_nums =
        gpu_values[static_cast<uint>(CuptiMetricsDefault::CUPQD_METRIC_KERNEL_ELAPSED_CLK_NUMS)];
    double core_frequency_hzs = gpu_values[static_cast<uint>(CuptiMetricsDefault::CUPQD_METRIC_CORE_FREQUENCY_HZS)];
    traced_records[records_size_after_sync + range_index].kernel_elapsed_time_in_ms =
        kernel_elapsed_clk_nums / core_frequency_hzs * 1000;  // from s to ms

    // user selected metrics
    uint user_metric_idx_begin = static_cast<uint>(CuptiMetricsDefault::CUPQD_METRIC_DEFAULT_TOTAL);
    uint metric_num = cupti_config_.metric_list.size();
    for (uint idx = user_metric_idx_begin; idx < metric_num; idx++) {
      traced_records[records_size_after_sync + range_index].metric_values.push_back(gpu_values[idx]);
    }
  }
  return true;
}
