...
bool KernelProfilerCUDA::reinit_with_metrics(const std::vector<std::string> metrics) {
  ...
  if (tool_ == ProfilingToolkit::event) {
    ...
    QD_TRACE("size of metric list : {} >>> {}", metrics.size(), metric_list_.size());
    return true;
  }
...
void KernelProfilerCUDA::trace(KernelProfilerBase::TaskHandle &task_handle,
  ...
  CUDADriver::get_instance().kernel_get_attribute(&register_per_thread,
                                                  CUfunction_attribute::CU_FUNC_ATTRIBUTE_NUM_REGS, kernel);
  CUDADriver::get_instance().kernel_get_attribute(&static_shared_mem_per_block,
                                                  CUfunction_attribute::CU_FUNC_ATTRIBUTE_SHARED_SIZE_BYTES, kernel);
  CUDADriver::get_instance().kernel_get_occupancy(&max_active_blocks_per_multiprocessor, kernel, block_size,
                                                  dynamic_smem_size);
  ...
  record.active_blocks_per_multiprocessor = max_active_blocks_per_multiprocessor;
...
void KernelProfilerCUDA::stop(KernelProfilerBase::TaskHandle handle) {
  if (tool_ == ProfilingToolkit::event) {
    ...
    CUDADriver::get_instance().event_elapsed_time(&record->kernel_elapsed_time_in_ms, record->start_event, handle);
    CUDADriver::get_instance().event_elapsed_time(&record->time_since_base, event_toolkit_->get_base_event(),
                                                  record->start_event);
...
bool KernelProfilerCUDA::record_kernel_attributes(void *kernel,
  ...
  traced_record.shared_mem_per_block = static_shared_mem_per_block + dynamic_smem_size;
  traced_record.grid_size = grid_size;
  traced_record.block_size = block_size;
  traced_record.active_blocks_per_multiprocessor = max_active_blocks_per_multiprocessor;
...
#if defined(QD_WITH_CUDA)
KernelProfilerBase::TaskHandle EventToolkit::start_with_handle(const std::string &kernel_name) {
  EventRecord record;
  record.name = kernel_name;
...
void EventToolkit::update_record(uint32_t records_size_after_sync,
  ...
  if (Timelines::get_instance().get_enabled()) {
    ...
    for (auto &record : traced_records) {
      ...
      timeline.insert_event(
          {record.name, /*param_name=begin*/ true, base_time_ + record.time_since_base * 1e-3, "cuda"});
