...
inline void update_mask(uint64 &mask, uint32 num_bits, uint32 offset) {
  uint64 new_mask = (((~(uint64)0) << (64 - num_bits)) >> (64 - offset - num_bits));
...
llvm::Value *TaskCodeGenLLVM::atomic_add_quant_int(llvm::Value *ptr,
  ...
  return call(fmt::format("atomic_add_partial_bits_b{}", physical_type->getIntegerBitWidth()), byte_ptr, bit_offset,
              tlctx->get_constant(qit->get_num_bits()), builder->CreateIntCast(value, physical_type, value_is_signed));
}
...
llvm::Value *TaskCodeGenLLVM::to_quant_fixed(llvm::Value *real, QuantFixedType *qfxt) {
  // Compute int(real * (1.0 / scale) + 0.5)
  auto compute_type = qfxt->get_compute_type();
  auto s = builder->CreateFPCast(tlctx->get_constant(1.0 / qfxt->get_scale()), tlctx->get_data_type(compute_type));
  ...
  scaled = call(fmt::format("rounding_prepare_f{}", data_type_bits(compute_type)), scaled);
...
void TaskCodeGenLLVM::store_quant_int(llvm::Value *ptr,
  ...
  call(fmt::format("{}set_partial_bits_b{}", atomic ? "atomic_" : "", physical_type->getIntegerBitWidth()), byte_ptr,
       bit_offset, tlctx->get_constant(qit->get_num_bits()), builder->CreateIntCast(value, physical_type, false));
}
...
void TaskCodeGenLLVM::store_quant_fixed(llvm::Value *ptr,
                                        ...
                                        llvm::Value *value,
                                        bool atomic) {
  store_quant_int(ptr, physical_type, qfxt->get_digits_type()->as<QuantIntType>(), to_quant_fixed(value, qfxt), atomic);
}

void TaskCodeGenLLVM::store_masked(llvm::Value *ptr, llvm::Type *ty, uint64 mask, llvm::Value *value, bool atomic) {
  if (!mask) {
    // do not store anything
    return;
  }
  uint64 full_mask = (~(uint64)0) >> (64 - ty->getIntegerBitWidth());
  if ((!atomic || compile_config.quant_opt_atomic_demotion) && ((mask & full_mask) == full_mask)) {
    builder->CreateStore(value, ptr);
    return;
  }
  call(fmt::format("{}set_mask_b{}", atomic ? "atomic_" : "", ty->getIntegerBitWidth()), ptr, tlctx->get_constant(mask),
       builder->CreateIntCast(value, ty, false));
}

llvm::Value *TaskCodeGenLLVM::get_exponent_offset(llvm::Value *exponent, QuantFloatType *qflt) {
  ...
  auto cond = builder->CreateICmp(llvm::CmpInst::Predicate::ICMP_NE, exponent, tlctx->get_constant(0));
  return builder->CreateSelect(cond, tlctx->get_constant(qflt->get_exponent_conversion_offset()),
                               tlctx->get_constant(0));
}

llvm::Value *TaskCodeGenLLVM::quant_int_or_quant_fixed_to_bits(llvm::Value *val,
                                                               Type *input_type,
  ...
  if (qit->get_num_bits() < val->getType()->getIntegerBitWidth()) {
    val = builder->CreateAnd(val,
                             tlctx->get_constant(qit->get_compute_type(), uint64((1ULL << qit->get_num_bits()) - 1)));
  }
...
void TaskCodeGenLLVM::visit(BitStructStoreStmt *stmt) {
  ...
  if (compile_config.quant_opt_atomic_demotion && stmt->ch_ids.size() == num_non_exponent_children) {
  ...
  for (int i = 0; i < stmt->ch_ids.size(); i++) {
    ...
    if (auto qflt = dtype->cast<QuantFloatType>()) {
      ...
      auto f32_bits = builder->CreateBitCast(val, llvm::Type::getInt32Ty(*llvm_context));
      ...
      if (qflt->get_digit_bits() < 23) {
        f32_bits = builder->CreateAdd(f32_bits, tlctx->get_constant(1 << (22 - qflt->get_digit_bits())));
      }

      auto exponent_bits = builder->CreateAShr(f32_bits, 23);
      exponent_bits = builder->CreateAnd(exponent_bits, tlctx->get_constant((1 << 8) - 1));
      auto value_bits = builder->CreateAShr(f32_bits, tlctx->get_constant(23 - qflt->get_digit_bits()));

      digit_bits = builder->CreateAnd(value_bits, tlctx->get_constant((1 << (qflt->get_digit_bits())) - 1));

      if (qflt->get_is_signed()) {
        // extract the sign bit
        auto sign_bit = builder->CreateAnd(f32_bits, tlctx->get_constant(0x80000000u));
        // insert the sign bit to digit bits
        digit_bits = builder->CreateOr(digit_bits, builder->CreateLShr(sign_bit, 31 - qflt->get_digit_bits()));
      }
      ...
      // bit_and.
      auto exp_non_zero = builder->CreateICmp(llvm::CmpInst::Predicate::ICMP_NE, exponent_bits, tlctx->get_constant(0));
      val = builder->CreateSelect(exp_non_zero, digit_bits, tlctx->get_constant(0));
  ...
  if (store_all_components && !has_shared_exponent) {
    ...
    for (int i = 0; i < stmt->ch_ids.size(); i++) {
      ...
      if (auto qflt = dtype->cast<QuantFloatType>()) {
        ...
        update_mask(mask, exponent_qit->get_num_bits(), bit_struct->get_member_bit_offset(exp));
...
void TaskCodeGenLLVM::store_quant_floats_with_shared_exponents(BitStructStoreStmt *stmt) {
  ...
  for (int i = 0; i < bit_struct->get_num_members(); i++) {
    ...
    for (auto user : exponent_users) {
      if (auto input = std::find(stmt->ch_ids.begin(), stmt->ch_ids.end(), user); input != stmt->ch_ids.end()) {
    ...
    auto max_exp_bits_to_store = builder->CreateSub(max_exp_bits, exponent_offset);

    max_exp_bits_to_store = call("max_i32", max_exp_bits_to_store, tlctx->get_constant(0));
    ...
    update_mask(mask, bit_struct->get_member_type(i)->as<QuantIntType>()->get_num_bits(), bit_offset);

    for (int c = 0; c < (int)exponent_users.size(); c++) {
      auto user = exponent_users[c];
      auto digits = extract_digits_from_f32_with_shared_exponent(floats[c], max_exp_bits);
      ...
      // round to nearest
      digits = builder->CreateAdd(digits, tlctx->get_constant(1 << (right_shift_bits - 1)));
      ...
      if (qflt->get_is_signed()) {
        auto float_bits = builder->CreateBitCast(floats[c], llvm::Type::getInt32Ty(*llvm_context));
      ...
      masked_val = builder->CreateOr(masked_val, val);
      auto num_digit_bits = qflt->get_digits_type()->as<QuantIntType>()->get_num_bits();
      update_mask(mask, num_digit_bits, digits_bit_offset);
    }
  }
  store_masked(llvm_val[stmt->ptr], physical_type, mask, masked_val, stmt->is_atomic);
}
...
llvm::Value *TaskCodeGenLLVM::extract_digits_from_f32_with_shared_exponent(llvm::Value *f, llvm::Value *shared_exp) {
  ...
  exp_non_zero = builder->CreateZExt(exp_non_zero, llvm::Type::getInt32Ty(*llvm_context));
...
llvm::Value *TaskCodeGenLLVM::extract_quant_float(llvm::Value *physical_value,
  ...
  auto digits = extract_quant_int(physical_value, tlctx->get_constant(digits_bit_offset),
                                  qflt->get_digits_type()->as<QuantIntType>());
  auto exponent = extract_quant_int(physical_value, tlctx->get_constant(exponent_bit_offset),
...
llvm::Value *TaskCodeGenLLVM::extract_quant_int(llvm::Value *physical_value,
  ...
  auto bit_end = builder->CreateAdd(bit_offset, tlctx->get_constant(qit->get_num_bits()));
  auto left = builder->CreateSub(tlctx->get_constant(physical_type->getIntegerBitWidth()), bit_end);
  auto right = builder->CreateSub(tlctx->get_constant(physical_type->getIntegerBitWidth()),
  ...
  return builder->CreateIntCast(step2, tlctx->get_data_type(qit->get_compute_type()), qit->get_is_signed());
}

llvm::Value *TaskCodeGenLLVM::reconstruct_quant_fixed(llvm::Value *digits, QuantFixedType *qfxt) {
...
llvm::Value *TaskCodeGenLLVM::reconstruct_quant_float(llvm::Value *input_digits,
  ...
  auto exponent_offset = tlctx->get_constant(qflt->get_exponent_conversion_offset());
  ...
  if (exponent_type->get_num_bits() < 8) {
    auto cond = builder->CreateICmp(llvm::CmpInst::Predicate::ICMP_NE, exponent_val, tlctx->get_constant(0));
    exponent_offset = builder->CreateSelect(cond, exponent_offset, tlctx->get_constant(0));
  }
  ...
  if (qflt->get_compute_type()->is_primitive(PrimitiveTypeID::f32)) {
    ...
    digits = builder->CreateAnd(digits, (1u << qflt->get_digits_type()->as<QuantIntType>()->get_num_bits()) - 1);

    llvm::Value *sign_bit = nullptr;

    if (shared_exponent) {
      if (qflt->get_is_signed()) {
        sign_bit = builder->CreateAnd(digits, tlctx->get_constant(1u << qflt->get_digit_bits()));
      ...
      auto num_leading_zeros = builder->CreateIntrinsic(llvm::Intrinsic::ctlz, {llvm::Type::getInt32Ty(*llvm_context)},
                                                        {digits, tlctx->get_constant(false)});
      auto extra_shift = builder->CreateSub(tlctx->get_constant(31 - qflt->get_digit_bits()), num_leading_zeros);
      exponent_offset = builder->CreateAdd(exponent_offset, extra_shift);

      if (!qflt->get_is_signed())
        exponent_offset = builder->CreateAdd(exponent_offset, tlctx->get_constant(1));

      auto digits_shift = builder->CreateSub(tlctx->get_constant(23 - qflt->get_digit_bits()), extra_shift);
      digits = builder->CreateShl(digits, digits_shift);
    } else {
      digits = builder->CreateShl(digits, tlctx->get_constant(23 - qflt->get_digit_bits()));
    }
    auto fraction_bits = builder->CreateAnd(digits, (1u << 23) - 1);

    exponent_val = builder->CreateAdd(exponent_val, exponent_offset);

    auto exponent_bits = builder->CreateShl(exponent_val, tlctx->get_constant(23));
    ...
    if (shared_exponent) {
      ...
      auto zero_exponent =
          builder->CreateICmp(llvm::CmpInst::Predicate::ICMP_EQ, input_exponent_val, tlctx->get_constant(0));
      auto zero_digits = builder->CreateICmp(llvm::CmpInst::Predicate::ICMP_EQ, input_digits, tlctx->get_constant(0));
      auto zero_output = builder->CreateOr(zero_exponent, zero_digits);
      f32_bits = builder->CreateSelect(zero_output, tlctx->get_constant(0), f32_bits);
    }
    ...
    return builder->CreateBitCast(f32_bits, llvm::Type::getFloatTy(*llvm_context));
  } else {
    QD_NOT_IMPLEMENTED;
  }
}
