...
#define STR(...) #__VA_ARGS__
  ...
  constexpr auto kArr = StrDelimSplitter<kDelimN>::make(STR(a, bc, def, gh), ',');
...
TEST(SERIALIZATION, Type) {
  ...
  auto *int32_type = TypeFactory::get_instance().get_primitive_type(PrimitiveTypeID::i32);
  auto *float32_type = TypeFactory::get_instance().get_primitive_type(PrimitiveTypeID::f32);
  auto *tensor_type = TypeFactory::get_instance().get_tensor_type({2, 2}, int32_type);
  auto *struct_type =
      TypeFactory::get_instance().get_struct_type({{int32_type, "i32", 0}, {tensor_type, "tensor", 4}}, "test");
  auto *pointer_type = TypeFactory::get_instance().get_pointer_type((Type *)struct_type, false);
  ...
  auto *quant_int6_type = TypeFactory::get_instance().get_quant_int_type(6, false, int32_type);
  auto *quant_int5_type = TypeFactory::get_instance().get_quant_int_type(5, false, int32_type);
  auto *quant_int7_type = TypeFactory::get_instance().get_quant_int_type(7, false, int32_type);
  auto *quant_float11_type =
      TypeFactory::get_instance().get_quant_float_type(quant_int5_type, quant_int6_type, float32_type);
  auto *quant_fixed_type = TypeFactory::get_instance().get_quant_fixed_type(quant_int7_type, float32_type, 2.0);
  auto *bit_struct_type = TypeFactory::get_instance().get_bit_struct_type(
      int32_type->as<PrimitiveType>(), {quant_int6_type, quant_float11_type, quant_fixed_type}, {0, 6, 11},
      {-1, -1, -1}, {{}, {}, {}});
  ...
  auto *quant_array_type =
      TypeFactory::get_instance().get_quant_array_type(int32_type->as<PrimitiveType>(), quant_int6_type, 3);
  // QuantArrayType doesn't have a deduplication mechanism, so we can't compare
  // the pointers
  EXPECT_EQ(bp.run(quant_array_type)->to_string(), quant_array_type->to_string());
