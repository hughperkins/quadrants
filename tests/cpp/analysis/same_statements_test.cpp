...
TEST(SameStatements, TestSameBlock) {
  ...
  auto global_load_addr = block->push_back<GlobalTemporaryStmt>(0, PrimitiveType::i32);
  auto global_load = block->push_back<GlobalLoadStmt>(global_load_addr);
  auto global_store_addr = block->push_back<GlobalTemporaryStmt>(4, PrimitiveType::i32);
  ...
  auto true_add = true_clause->push_back<BinaryOpStmt>(BinaryOpType::add, global_load, true_one);
  auto true_store = true_clause->push_back<GlobalStoreStmt>(global_store_addr, true_add);
  ...
  auto false_add = false_clause->push_back<BinaryOpStmt>(BinaryOpType::add, global_load, false_one);
  auto false_store = false_clause->push_back<GlobalStoreStmt>(global_store_addr, false_add);
  ...
  EXPECT_TRUE(!irpass::analysis::same_statements(global_load_addr, global_store_addr));

  EXPECT_TRUE(irpass::analysis::same_statements(if_stmt->true_statements.get(), if_stmt->false_statements.get()));
...
TEST(SameStatements, TestSameAssert) {
  ...
  auto one = block->push_back<ConstStmt>(TypedConstant(1));
  auto assert_zero_a = block->push_back<AssertStmt>(zero, "a", std::vector<Stmt *>());
  auto assert_zero_a2 = block->push_back<AssertStmt>(zero, "a", std::vector<Stmt *>());
  auto assert_zero_b = block->push_back<AssertStmt>(zero, "b", std::vector<Stmt *>());
  auto assert_one_a = block->push_back<AssertStmt>(one, "a", std::vector<Stmt *>());
  auto assert_zero_a_one = block->push_back<AssertStmt>(zero, "a", std::vector<Stmt *>(1, one));
  auto assert_zero_a_one2 = block->push_back<AssertStmt>(zero, "a", std::vector<Stmt *>(1, one));
  auto assert_zero_a_zero = block->push_back<AssertStmt>(zero, "a", std::vector<Stmt *>(1, zero));
  auto assert_one_a_zero = block->push_back<AssertStmt>(one, "a", std::vector<Stmt *>(1, zero));
  ...
  EXPECT_TRUE(!irpass::analysis::same_statements(assert_zero_a, assert_zero_a_one));
  EXPECT_TRUE(irpass::analysis::same_statements(assert_zero_a_one, assert_zero_a_one2));
  EXPECT_TRUE(!irpass::analysis::same_statements(assert_zero_a_one, assert_zero_a_zero));
  EXPECT_TRUE(!irpass::analysis::same_statements(assert_zero_a_one, assert_one_a_zero));
}
...
TEST(SameStatements, TestSameSnodeLookup) {
  ...
  auto lookup_activate = block->push_back<SNodeLookupStmt>(&root, get_root, zero, true);
  auto get_child = block->push_back<GetChStmt>(lookup_activate, 0);
  auto lookup_child = block->push_back<SNodeLookupStmt>(&child, get_child, zero, false);
...
TEST(SameStatements, TestSameValue) {
  ...
  auto true_add = true_clause->push_back<BinaryOpStmt>(BinaryOpType::add, one, true_rand);
  if_stmt->set_true_statements(std::move(true_clause));

  auto false_clause = std::make_unique<Block>();
  auto false_rand = false_clause->push_back<RandStmt>(PrimitiveType::i32);
  auto false_add = false_clause->push_back<BinaryOpStmt>(BinaryOpType::add, one, false_rand);
  if_stmt->set_false_statements(std::move(false_clause));
  ...
  EXPECT_TRUE(!irpass::analysis::same_statements(true_add, false_add, std::nullopt));
  EXPECT_TRUE(
      irpass::analysis::same_statements(true_add, false_add, std::make_optional<std::unordered_map<int, int>>()));

  EXPECT_TRUE(!irpass::analysis::same_value(true_add, false_add, std::make_optional<std::unordered_map<int, int>>()));
}
...
TEST(SameStatements, TestSameLoopIndex) {
  ...
  auto range_for =
      block->push_back<RangeForStmt>(zero, four, std::make_unique<Block>(), 1, 1, 1, false)->as<RangeForStmt>();
