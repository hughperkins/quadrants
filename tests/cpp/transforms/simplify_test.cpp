...
TEST(Simplify, SimplifyLinearizedWithTrivialInputs) {
  ...
  auto linearized_zero = block->push_back<LinearizeStmt>(std::vector<Stmt *>(2, zero), std::vector<int>({8, 4}));
  [[maybe_unused]] auto lookup2 = block->push_back<SNodeLookupStmt>(root.ch[0].get(), get_child, linearized_zero, true);
