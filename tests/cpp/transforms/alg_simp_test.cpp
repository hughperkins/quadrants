...
TEST_F(AlgebraicSimplicationTest, SimplifyAddZero) {
  ...
  auto add = block->push_back<BinaryOpStmt>(BinaryOpType::add, global_load, zero);
...
TEST_F(AlgebraicSimplicationTest, SimplifyMultiplyOne) {
  ...
  auto global_load_addr = block->push_back<GlobalTemporaryStmt>(0, PrimitiveType::f32);
  auto global_load = block->push_back<GlobalLoadStmt>(global_load_addr);
  auto one = block->push_back<ConstStmt>(TypedConstant(1.0f));
  auto mul1 = block->push_back<BinaryOpStmt>(BinaryOpType::mul, one, global_load);
  ...
  auto global_store_addr = block->push_back<GlobalTemporaryStmt>(4, PrimitiveType::f32);
  [[maybe_unused]] auto global_store = block->push_back<GlobalStoreStmt>(global_store_addr, sub);
...
TEST_F(AlgebraicSimplicationTest, SimplifyMultiplyZeroFastMath) {
  ...
  auto global_load = block->push_back<GlobalLoadStmt>(global_load_addr);
  auto zero = block->push_back<ConstStmt>(TypedConstant(0));
  auto mul = block->push_back<BinaryOpStmt>(BinaryOpType::mul, global_load, zero);
  auto one = block->push_back<ConstStmt>(TypedConstant(1));
  auto add = block->push_back<BinaryOpStmt>(BinaryOpType::add, mul, one);
  auto global_store_addr = block->push_back<GlobalTemporaryStmt>(4, PrimitiveType::i32);
  [[maybe_unused]] auto global_store = block->push_back<GlobalStoreStmt>(global_store_addr, add);
  ...
  global_load_addr = block->push_back<GlobalTemporaryStmt>(8, PrimitiveType::f32);
  ...
  global_store_addr = block->push_back<GlobalTemporaryStmt>(12, PrimitiveType::f32);
...
TEST_F(AlgebraicSimplicationTest, SimplifyAndMinusOne) {
  ...
  auto and_result = block->push_back<BinaryOpStmt>(BinaryOpType::bit_and, minus_one, global_load);
  auto global_store_addr = block->push_back<GlobalTemporaryStmt>(4, PrimitiveType::i32);
