...
- a backend with DLPack support: `cpu` (`x64`/`arm64`), `cuda`, `amdgpu`, or `metal`. Vulkan is not supported, mostly because it is currently irrelevant: Vulkan-backed DLPack tensors are not processed by any well-known scientific computing library (torch, numpy, etc.);
